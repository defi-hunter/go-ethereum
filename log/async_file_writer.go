...
func NewAsyncFileWriter(filePath string, bufSize int64, max_file_num int) *AsyncFileWriter {
...
func (w *AsyncFileWriter) loadOldLogList() {
	file, err := os.Open(w.filePath+".lst")
	if err != nil {
		return
	}
	...
	for scanner.Scan() {
		w.logList = append(w.logList, scanner.Text())
	}
}

func (w *AsyncFileWriter) updateLogFileList() {
	var rm []string
	maxLogNum := w.max_file_num
	if maxLogNum <= 0 {
		maxLogNum = 48
	}

	if len(w.logList) > maxLogNum {
		rm = w.logList[:len(w.logList)-maxLogNum]
		w.logList = w.logList[len(rm):]
	}
	...
	var buff string
	for _, f := range w.logList {
		buff += fmt.Sprintf("%s\n", f)
	}

	logListFile := w.filePath+".lst"

	os.Remove(logListFile)
	file, err := os.OpenFile(logListFile, os.O_CREATE|os.O_RDWR, 0644)
	if err != nil {
		return
	}
...
func (w *AsyncFileWriter) rotateFile() {
	...
	case <-w.hourTicker.C:
		...
		w.logList = append(w.logList, w.currentLogFile)
		w.updateLogFileList()
